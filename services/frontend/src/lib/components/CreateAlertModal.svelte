...
<script lang="ts">
    ...
    function addCondition() {
        conditions = [...conditions, { type: "status_code", operator: "equals", value: "" }];
    }
    ...
    function updateCondition(index: number, field: keyof AlertCondition, value: any) {
    ...
    function handleSubmit() {
        ...
        const processedConditions = conditions.map(cond => ({
            ...cond,
            value: cond.type === "response_time" || cond.type === "status_code"
                ? Number(cond.value)
                : cond.value
        }));
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            >
                <div class="flex items-center justify-between mb-6">
                    <h3 class="text-2xl font-bold text-gray-900">Create Alert Rule</h3>
                    <button
                        onclick={handleClose}
                ...
                <div class="space-y-6">
                                ...
                                <div class="p-4 border border-gray-200 rounded-lg bg-gray-50">
                                    <div class="grid grid-cols-12 gap-3 items-start">
                                        ...
                                        <div class="col-span-4">
                                            ...
                                            {:else}
                                                <input
                                                    type={condition.type === "response_time" || condition.type === "status_code" ? "number" : "text"}
                                                    value={condition.value}
                                                    oninput={(e) => updateCondition(index, "value", e.currentTarget.value)}
                                                    placeholder={condition.type === "response_time" ? "ms" : condition.type === "status_code" ? "e.g., 404" : "value"}
                                                    class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                                                    disabled={createAlertMutation.isPending}
                                                />
                                            {/if}
                                        ...
                                        <div class="col-span-1">
                                            <button
                                                ...
                                                disabled={createAlertMutation.isPending || conditions.length === 1}
                    ...
                    <div class="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                        <p class="text-sm text-blue-800">
                            <strong>Note:</strong> All conditions must match (AND logic) for the alert to trigger.
                            The cooldown period prevents alert spam by limiting how often the same rule can trigger.
                        </p>
                    </div>
                </div>
