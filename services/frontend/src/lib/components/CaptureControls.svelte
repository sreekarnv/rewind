...
<div class="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg overflow-hidden border border-gray-100">
    <div class="bg-gradient-to-r from-purple-50 to-pink-50 px-6 py-4 border-b border-gray-200">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="text-lg font-semibold bg-gradient-to-r from-purple-600 to-pink-600 bg-clip-text text-transparent">Capture Agent</h3>
                    <p class="text-sm text-gray-600">Control packet capture</p>
                </div>
            </div>
            <div class="flex items-center gap-2">
                {#if isRunning}
                    <span class="inline-flex items-center px-4 py-2 rounded-xl text-sm font-semibold bg-gradient-to-r from-green-500 to-emerald-500 text-white shadow-md">
                ...
                {:else if isTransitioning}
                    <span class="inline-flex items-center px-4 py-2 rounded-xl text-sm font-semibold bg-gradient-to-r from-yellow-500 to-orange-500 text-white shadow-md">
                ...
                {:else if hasError}
                    <span class="inline-flex items-center px-4 py-2 rounded-xl text-sm font-semibold bg-gradient-to-r from-red-500 to-red-600 text-white shadow-md">
                        <span class="w-2 h-2 bg-white rounded-full mr-2"></span>
                        Error
                    </span>
                {:else}
                    <span class="inline-flex items-center px-4 py-2 rounded-xl text-sm font-semibold bg-gradient-to-r from-gray-400 to-gray-500 text-white shadow-md">
                        <span class="w-2 h-2 bg-white rounded-full mr-2"></span>
                        Stopped
                    </span>
                {/if}
    ...
    <div class="p-6">
        ...
        {#if error || captureState?.error}
            <div class="mb-6 bg-gradient-to-r from-yellow-50 to-orange-50 border border-yellow-200/50 rounded-xl p-5 shadow-sm">
                <div class="flex gap-4">
                    ...
                    <div class="flex-1">
                        ...
                        <div class="text-sm text-yellow-800">
                            ...
                            <ol class="list-decimal list-inside mt-2 space-y-2 text-xs bg-yellow-100/70 p-4 rounded-xl border border-yellow-200">
                                <li class="ml-2">
                                    Open a terminal/command prompt <strong class="text-yellow-900">as Administrator</strong>
                                </li>
                                <li class="ml-2">
                                    Navigate to: <code class="bg-yellow-200 px-2 py-0.5 rounded font-mono text-yellow-900">services/capture-agent/build/Release</code>
                                </li>
                                <li class="ml-2">
                                    Run: <code class="bg-yellow-200 px-2 py-0.5 rounded font-mono text-yellow-900">./capture-agent.exe --config ../../config/config.yaml</code>
                                </li>
                                <li class="ml-2">Select your network interface when prompted</li>
                            </ol>
                            <p class="mt-3 text-xs text-yellow-700 flex items-center gap-2">
                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                </svg>
                                The UI will automatically show captured sessions once the agent is running.
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div class="grid grid-cols-2 gap-4 mb-6">
                {#if captureState.pid}
                    <div class="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-4 border border-blue-100">
                        <p class="text-xs text-blue-600 font-medium mb-1 flex items-center gap-1">
                            <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 3v2m6-2v2M9 19v2m6-2v2M5 9H3m2 6H3m18-6h-2m2 6h-2M7 19h10a2 2 0 002-2V7a2 2 0 00-2-2H7a2 2 0 00-2 2v10a2 2 0 002 2zM9 9h6v6H9V9z" />
                            </svg>
                            Process ID
                        </p>
                        <p class="font-mono text-lg font-bold text-gray-900">{captureState.pid}</p>
                    </div>
                {/if}
                {#if captureState.uptime}
                    <div class="bg-gradient-to-r from-purple-50 to-pink-50 rounded-xl p-4 border border-purple-100">
                        <p class="text-xs text-purple-600 font-medium mb-1 flex items-center gap-1">
                            <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div class="text-sm text-gray-700">
                    {#if isRunning}
                        <p class="font-medium text-green-700">
                            Capture agent is actively capturing HTTP traffic. Click "Stop Capture" to end the session.
                        </p>
                    {:else if isStopped}
                        <p class="mb-2">
                            Click "Start Capture" to launch the agent from the UI, or run it manually in a terminal.
                        </p>
                        <p class="text-gray-600 text-xs">
                            <strong class="text-gray-700">Note:</strong> Packet capture requires administrator/root permissions. If the UI button fails, run the agent manually with elevated privileges.
                        </p>
                    {:else if hasError}
                        <p>
                            The agent couldn't start automatically. This is normal - packet capture requires elevated permissions.
                        </p>
                    {/if}
                </div>
            </div>
        ...
        {#if isRunning || isTransitioning}
            <div class="mt-4">
                ...
                >
                    ...
                    <span>{showTerminal ? 'Hide Agent Output' : 'Show Agent Output'}</span>
                </button>
            </div>
        {/if}
