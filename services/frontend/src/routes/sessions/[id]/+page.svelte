    ...
    <div class="card">
        ...
        <div class="mt-4 pt-4 border-t">
            <p class="text-sm text-gray-500">
                Started: <span class="font-medium text-gray-900"
                    >{formatTimestamp(session.startTime)}</span
                >
            </p>
            <p class="text-sm text-gray-500">
                Ended: <span class="font-medium text-gray-900"
                    >{formatTimestamp(session.endTime)}</span
                >
            </p>
        </div>
    </div>
                ...
                <div class="space-y-2 max-h-[600px] overflow-y-auto">
                    {#each transactions as transaction, index}
                        <button
                            ...
                            class="w-full text-left p-3 rounded-md transition-colors {selectedTransaction ===
                            index
                        ...
                        <div class="grid grid-cols-2 gap-4 text-sm">
                            {#if currentTransaction.requestTime}
                                <div>
                                    ...
                                    <p class="font-medium text-gray-900">
                                        {formatTimestamp(
                                            currentTransaction.requestTime,
                                        )}
                                    </p>
                                </div>
                            {/if}
                            {#if currentTransaction.responseTime}
                                <div>
                                    ...
                                    <p class="font-medium text-gray-900">
                                        {formatTimestamp(
                                            currentTransaction.responseTime,
                                        )}
                                    </p>
                                </div>
                            {/if}
                            ...
                            {#if currentTransaction.response?.length}
                                <div>
                                    ...
                                    <p class="font-medium text-gray-900">
                                        {formatBytes(
                                            currentTransaction.response.length,
                                        )}
                                    </p>
                                </div>
                            {/if}
                        ...
                        {#if !currentTransaction.response}
                            ...
                            >
                                <p class="text-sm text-yellow-800">
                                    ⚠️ This transaction has no response
                                    (incomplete capture)
                                </p>
                            </div>
                        {/if}
