...
<div class="space-y-6">
	...
	<div>
		...
		<p class="mt-1 text-sm text-gray-500">Aggregated metrics from captured HTTP traffic</p>
	</div>
	...
	{#if stats}
		...
		<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
			...
			<div class="card">
				<h2 class="text-lg font-semibold text-gray-900 mb-4">HTTP Methods</h2>
				<div class="space-y-3">
					{#each methodEntries as [method, count]}
						{@const percentage = getPercentage(count, totalRequests)}
						<div>
			...
			<div class="card">
				<h2 class="text-lg font-semibold text-gray-900 mb-4">Status Codes</h2>
				<div class="space-y-3">
		...
		<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
			<!-- Methods Table -->
			<div class="card">
				<h3 class="text-base font-semibold text-gray-900 mb-3">Method Breakdown</h3>
				<div class="overflow-x-auto">
					<table class="min-w-full divide-y divide-gray-200">
						...
						<tbody class="divide-y divide-gray-200">
							{#each methodEntries as [method, count]}
								<tr>
									<td class="px-3 py-2 text-sm font-medium text-gray-900">{method}</td>
									<td class="px-3 py-2 text-sm text-gray-500 text-right">{count}</td>
									<td class="px-3 py-2 text-sm text-gray-500 text-right"
										>{getPercentage(count, totalRequests)}%</td
									>
								</tr>
							{/each}
						</tbody>
			...
			<!-- Status Codes Table -->
			<div class="card">
				<h3 class="text-base font-semibold text-gray-900 mb-3">Status Code Breakdown</h3>
				<div class="overflow-x-auto">
					<table class="min-w-full divide-y divide-gray-200">
						<thead>
							<tr>
						...
						<tbody class="divide-y divide-gray-200">
							{#each statusEntries as [status, count]}
								<tr>
									<td class="px-3 py-2 text-sm font-medium text-gray-900">{status}</td>
									<td class="px-3 py-2 text-sm text-gray-500 text-right">{count}</td>
									<td class="px-3 py-2 text-sm text-gray-500 text-right"
										>{getPercentage(count, totalRequests)}%</td
									>
