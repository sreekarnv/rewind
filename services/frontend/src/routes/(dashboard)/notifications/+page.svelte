...
<script lang="ts">
    ...
    import type { Notification, AlertSeverity, NotificationStatus } from "$lib/types";
    ...
    const markAsDismissedMutation = createMutation(() => ({
        mutationFn: async (id: string) => {
            const response = await client.api.v1.notifications({ id }).dismiss.patch();
    ...
    function handleViewSession(sessionId: string, notificationId: string, status: NotificationStatus) {
        if (status === "unread") {
...
<div class="max-w-7xl mx-auto p-4 md:p-8">
    ...
    <div class="mb-8">
        ...
        <div class="flex gap-2">
            <button
                ...
                class="px-4 py-2 rounded-lg font-medium transition-all {statusFilter === undefined
            ...
            <button
                ...
                class="px-4 py-2 rounded-lg font-medium transition-all {statusFilter === 'unread'
                    ? 'bg-purple-100 text-purple-700 border-2 border-purple-300'
                    : 'bg-white text-gray-700 border border-gray-300 hover:bg-gray-50'}"
            >
                Unread ({unreadCount})
            ...
            <button
                ...
                class="px-4 py-2 rounded-lg font-medium transition-all {statusFilter === 'read'
                    ? 'bg-purple-100 text-purple-700 border-2 border-purple-300'
                    : 'bg-white text-gray-700 border border-gray-300 hover:bg-gray-50'}"
            >
                Read
            ...
            <button
                ...
                class="px-4 py-2 rounded-lg font-medium transition-all {statusFilter === 'dismissed'
                    ? 'bg-purple-100 text-purple-700 border-2 border-purple-300'
                    : 'bg-white text-gray-700 border border-gray-300 hover:bg-gray-50'}"
            >
                Dismissed
    ...
    {:else if notificationsQuery.isError}
        <div class="bg-red-50 border border-red-200 rounded-xl p-6 text-center">
            <p class="text-red-700">Failed to load notifications. Please try again.</p>
        </div>
    ...
    {:else}
        <div class="space-y-3">
            {#each notifications as notification (notification._id)}
                <div
                    class="bg-white/80 backdrop-blur-sm rounded-xl shadow border {notification.status === 'unread'
                    ...
                    <div class="p-5">
                        <div class="flex items-start gap-4">
                                ...
                                <div class="flex items-start justify-between gap-3 mb-2">
                                    <div class="flex-1">
                                        <div class="flex items-center gap-2 mb-1">
                                            ...
                                            <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium {getSeverityColor(notification.severity)} border">
                                        ...
                                        <div class="flex items-center gap-3 text-sm text-gray-600">
                                            <span class="font-mono font-semibold">{notification.sessionData.method}</span>
                                            <span>•</span>
                                            <span class="truncate max-w-md">{notification.sessionData.uri}</span>
                                            {#if notification.sessionData.statusCode}
                                                <span>•</span>
                                                <span>Status: {notification.sessionData.statusCode}</span>
                                            {/if}
                                        </div>
                            ...
                            <div class="flex flex-col gap-2">
                                <button
                                    onclick={() => handleViewSession(notification.sessionId, notification._id, notification.status)}
                                ...
                                {#if notification.status !== "dismissed"}
                                    <button
                                        onclick={() => markAsDismissedMutation.mutate(notification._id)}
                                        class="px-3 py-1.5 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors text-sm font-medium whitespace-nowrap"
                                    >
                                        Dismiss
                                    </button>
